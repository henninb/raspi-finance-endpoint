...
if ! validate_and_create_keystore; then
  ...
  log_error "Cannot proceed with GCP deployment without valid SSL configuration."
  log_error ""
...
if ssh gcp-api 'curl -k -f -s https://localhost:8443/actuator/health >/dev/null 2>&1'; then
...
log "Testing domain name resolution with forced DNS..."
if curl --resolve finance.bhenning.com:443:34.132.189.202 -f -s --connect-timeout 10 https://finance.bhenning.com/actuator/health >/dev/null 2>&1; then
    log "✓ Domain name access working: https://finance.bhenning.com/"
else
    log "⚠ Domain name access test failed"
fi
