...
-- Account --
-------------
CREATE TABLE IF NOT EXISTS stage.t_account
(
...
----------------------------
CREATE TABLE IF NOT EXISTS stage.t_validation_amount(
    ...
    CONSTRAINT fk_account_id FOREIGN KEY (account_id) REFERENCES stage.t_account (account_id) ON DELETE CASCADE
);

--------------
-- User     --
--------------
CREATE TABLE IF NOT EXISTS stage.t_user
(
...
-- Role     --
--------------
CREATE TABLE IF NOT EXISTS stage.t_role
(
...
---------------------------
CREATE TABLE IF NOT EXISTS stage.t_transaction_categories
(
...
-- ReceiptImage  --
-------------------
CREATE TABLE IF NOT EXISTS stage.t_receipt_image
(
...
CREATE TABLE IF NOT EXISTS stage.t_transaction
(
    ...
    CONSTRAINT ck_reoccurring_type CHECK (reoccurring_type IN
                                          ('annually', 'biannually', 'fortnightly', 'monthly', 'quarterly', 'onetime',
                                           'undefined')),
    CONSTRAINT fk_account_id_account_name_owner FOREIGN KEY (account_id, account_name_owner, account_type) REFERENCES stage.t_account (account_id, account_name_owner, account_type) ON DELETE CASCADE,
    CONSTRAINT fk_receipt_image FOREIGN KEY (receipt_image_id) REFERENCES stage.t_receipt_image (receipt_image_id) ON DELETE CASCADE,
    CONSTRAINT fk_category FOREIGN KEY (category) REFERENCES stage.t_category (category_name) ON DELETE CASCADE
);
...
-- Payment --
-------------
CREATE TABLE IF NOT EXISTS stage.t_payment
(
    ...
    CONSTRAINT fk_guid_source FOREIGN KEY (guid_source) REFERENCES stage.t_transaction (guid) ON DELETE CASCADE,
...
-- parameter    --
-------------
CREATE TABLE IF NOT EXISTS stage.t_parameter
(
...
-- ALTER TABLE stage.t_parameter ADD COLUMN active_status BOOLEAN NOT NULL DEFAULT TRUE;
...
SELECT setval('stage.t_transaction_transaction_id_seq', (SELECT MAX(transaction_id) FROM stage.t_transaction) + 1);
SELECT setval('stage.t_payment_payment_id_seq', (SELECT MAX(payment_id) FROM stage.t_payment) + 1);
SELECT setval('stage.t_account_account_id_seq', (SELECT MAX(account_id) FROM stage.t_account) + 1);
SELECT setval('stage.t_category_category_id_seq', (SELECT MAX(category_id) FROM stage.t_category) + 1);
SELECT setval('stage.t_description_description_id_seq', (SELECT MAX(description_id) FROM stage.t_description) + 1);
SELECT setval('stage.t_parameter_parameter_id_seq', (SELECT MAX(parameter_id) FROM stage.t_parameter) + 1);
SELECT setval('stage.t_validation_amount_validation_id_seq', (SELECT MAX(validation_id) FROM stage.t_validation_amount) + 1);
...
DROP TRIGGER IF EXISTS tr_insert_transaction_categories ON stage.t_transaction_categories;
CREATE TRIGGER tr_insert_transaction_categories
    BEFORE INSERT
    ON stage.t_transaction_categories
    FOR EACH ROW
EXECUTE PROCEDURE fn_insert_transaction_categories();

DROP TRIGGER IF EXISTS tr_update_transaction_categories ON stage.t_transaction_categories;
