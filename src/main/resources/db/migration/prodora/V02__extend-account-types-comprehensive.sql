-- V02: Extend AccountType enum with comprehensive account types (Oracle version)
...
ALTER TABLE t_account
DROP CONSTRAINT ck_account_type_lowercase;
...
CREATE INDEX idx_account_type ON t_account(account_type);

-- Add index for account category queries (will be useful for reporting)
CREATE INDEX idx_account_active_type ON t_account(active_status, account_type);