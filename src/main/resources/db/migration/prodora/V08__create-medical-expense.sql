-- Medical Expense Table Creation for Production Oracle Environment
...
COMMENT ON TABLE t_medical_expense IS 'Medical expenses linked to transactions with comprehensive tracking';
COMMENT ON COLUMN t_medical_expense.medical_expense_id IS 'Primary key for medical expense records';
COMMENT ON COLUMN t_medical_expense.transaction_id IS 'Foreign key to t_transaction (1:1 relationship)';
COMMENT ON COLUMN t_medical_expense.provider_id IS 'Foreign key to t_medical_provider';
COMMENT ON COLUMN t_medical_expense.family_member_id IS 'Foreign key to t_family_member for tracking which family member';
COMMENT ON COLUMN t_medical_expense.service_date IS 'Date medical service was provided (different from payment date)';
COMMENT ON COLUMN t_medical_expense.billed_amount IS 'Original amount billed by provider';
COMMENT ON COLUMN t_medical_expense.insurance_discount IS 'Insurance negotiated discount amount';
COMMENT ON COLUMN t_medical_expense.insurance_paid IS 'Amount paid by insurance';
COMMENT ON COLUMN t_medical_expense.patient_responsibility IS 'Amount patient is responsible to pay';
COMMENT ON COLUMN t_medical_expense.is_out_of_network IS 'Whether provider is out of insurance network (0=false, 1=true)';
COMMENT ON COLUMN t_medical_expense.claim_status IS 'Status of insurance claim processing';