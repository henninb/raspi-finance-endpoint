...
open class AccountController @Autowired constructor(private var accountService: AccountService) {
    ...
    @PostMapping(path = [("/account/insert")], consumes = [("application/json")], produces = [("application/json")])
    ...
    @PatchMapping(path = [("/account/update/{accountNameOwner}")], consumes = [("application/json-patch+json")], produces = [("application/json")])
