...
@ControllerAdvice(assignableTypes = [ValidationAmountController::class])
@Order(1)
class ValidationAmountControllerAdvice {
    companion object {
    ...
    fun handleMessageNotReadableException(throwable: HttpMessageNotReadableException): ResponseEntity<Map<String, String>> {
        logger.error("ValidationAmount - Message not readable: ${throwable.message}", throwable)
    ...
    fun handleValidationException(throwable: ValidationException): ResponseEntity<Map<String, String>> {
        logger.error("ValidationAmount - Validation error: ${throwable.message}", throwable)
