...
@ControllerAdvice(assignableTypes = [TransferController::class])
@Order(1)
class TransferControllerAdvice {
    ...
    fun handleMessageNotReadableException(throwable: HttpMessageNotReadableException): ResponseEntity<Map<String, String>> {
        logger.error("Transfer - Message not readable: ${throwable.message}", throwable)
    ...
    fun handleValidationException(throwable: ValidationException): ResponseEntity<Map<String, String>> {
        logger.error("Transfer - Validation error: ${throwable.message}", throwable)
