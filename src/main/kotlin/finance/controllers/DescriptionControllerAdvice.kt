...
@ControllerAdvice(assignableTypes = [DescriptionController::class])
@Order(1)
class DescriptionControllerAdvice {
    ...
    fun handleMessageNotReadableException(throwable: HttpMessageNotReadableException): ResponseEntity<Map<String, String>> {
        logger.error("Description - Message not readable: ${throwable.message}", throwable)
    ...
    fun handleValidationException(throwable: ValidationException): ResponseEntity<Map<String, String>> {
        logger.error("Description - Validation error: ${throwable.message}", throwable)
