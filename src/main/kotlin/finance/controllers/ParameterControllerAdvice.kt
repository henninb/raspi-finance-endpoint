...
@ControllerAdvice(assignableTypes = [ParameterController::class])
@Order(1)
class ParameterControllerAdvice {
    ...
    fun handleMessageNotReadableException(throwable: HttpMessageNotReadableException): ResponseEntity<Map<String, String>> {
        logger.error("Parameter - Message not readable: ${throwable.message}", throwable)
    ...
    fun handleValidationException(throwable: ValidationException): ResponseEntity<Map<String, String>> {
        logger.error("Parameter - Validation error: ${throwable.message}", throwable)
