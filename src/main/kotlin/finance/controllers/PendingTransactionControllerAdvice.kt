...
@ControllerAdvice(assignableTypes = [PendingTransactionController::class])
@Order(1)
class PendingTransactionControllerAdvice {
    ...
    fun handleMessageNotReadableException(throwable: HttpMessageNotReadableException): ResponseEntity<Map<String, String>> {
        logger.error("PendingTransaction - Message not readable: ${throwable.message}", throwable)
    ...
    fun handleValidationException(throwable: ValidationException): ResponseEntity<Map<String, String>> {
        logger.error("PendingTransaction - Validation error: ${throwable.message}", throwable)
