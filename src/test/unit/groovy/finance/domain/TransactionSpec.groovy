...
'''
    ...
    void 'test validation invalid #invalidField has error expectedError'() {
        ...
        'description'      | UUID.randomUUID()                      | 1003      | AccountType.Credit | 'one-chase_brian'  | new Date(1553645394) | ''               | 'online' | new BigDecimal('-3.11').setScale(2, RoundingMode.HALF_UP)  | TransactionState.Cleared | false       | ReoccurringType.Undefined | 'no notes' | 'size must be between 1 and 75'            | 1
        'accountNameOwner' | UUID.randomUUID()                      | 1003      | AccountType.Credit | 'one.chase_brian'  | new Date(1553645394) | 'target.com'     | 'online' | new BigDecimal('13.14').setScale(2, RoundingMode.HALF_UP)  | TransactionState.Cleared | false       | ReoccurringType.Undefined | ''         | 'must be alpha separated by an underscore' | 1
        'accountNameOwner' | UUID.randomUUID()                      | 1003      | AccountType.Credit | 'one -chase_brian' | new Date(1553645394) | 'target.com'     | 'online' | new BigDecimal('13.14').setScale(2, RoundingMode.HALF_UP)  | TransactionState.Cleared | false       | ReoccurringType.Undefined | ''         | 'must be alpha separated by an underscore' | 1
        'accountNameOwner' | UUID.randomUUID()                      | 1003      | AccountType.Credit | 'brian'            | new Date(1553645394) | 'target.com'     | 'online' | new BigDecimal('13.14').setScale(2, RoundingMode.HALF_UP)  | TransactionState.Cleared | false       | ReoccurringType.Undefined | ''         | 'must be alpha separated by an underscore' | 1
        'category'         | UUID.randomUUID()                      | 1003      | AccountType.Credit | 'one-chase_brian'  | new Date(1553645394) | 'Cafe Roale'     | 'onliné' | new BigDecimal('3.14').setScale(2, RoundingMode.HALF_UP)   | TransactionState.Cleared | false       | ReoccurringType.Undefined | 'no notes' | 'must be alphanumeric no space'            | 1
        'category'         | UUID.randomUUID()                      | 1003      | AccountType.Credit | 'one-chase_brian'  | new Date(1553645394) | 'Cafe Roale'     | 'Online' | new BigDecimal('3.14').setScale(2, RoundingMode.HALF_UP)   | TransactionState.Cleared | false       | ReoccurringType.Undefined | 'no notes' | 'must be alphanumeric no space'            | 1
        'amount'           | UUID.randomUUID()                      | 1003      | AccountType.Credit | 'one-chase_brian'  | new Date(1553645394) | 'Cafe Roale'     | 'online' | new BigDecimal('3.1412')                                   | TransactionState.Cleared | false       | ReoccurringType.Undefined | 'no notes' | 'must be dollar precision'                 | 1
        'transactionDate'  | UUID.randomUUID()                      | 1003      | AccountType.Credit | 'one-chase_brian'  | new Date(946684700)  | 'Cafe Roale'     | 'online' | new BigDecimal('3.14').setScale(2, RoundingMode.HALF_UP)   | TransactionState.Cleared | false       | ReoccurringType.Undefined | 'no notes' | 'date must be greater than 1/1/2000.'      | 1
    }
}
