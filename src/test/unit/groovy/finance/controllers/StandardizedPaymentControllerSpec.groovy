...
class StandardizedPaymentControllerSpec extends Specification {
    ...
    def "selectAllPayments returns list when payments exist"() {
        given:
        ...
        response.body.any { it.destinationAccount == "visa_janice" }
    }

    def "selectAllPayments returns empty list when no payments"() {
        given:
    ...
    def "selectAllPayments throws 500 on repository error"() {
        given:
        paymentService.findAllPayments() >> { throw new RuntimeException("Database error") }

        when:
        controller.selectAllPayments()

        then:
        def ex = thrown(ResponseStatusException)
        ex.statusCode == HttpStatus.INTERNAL_SERVER_ERROR
        ex.reason.contains("Failed to retrieve payments")
    }

    def "updatePayment returns updated payment when successful"() {
        given:
        Payment patchPayment = createTestPayment(1L, "checking_brian", "visa_brian", new BigDecimal("150.00"))
        and:
        paymentService.updatePayment(1L, patchPayment) >> patchPayment

        when:
        ResponseEntity<Payment> response = controller.updatePayment(1L, patchPayment)
    ...
    def "updatePayment throws 409 on data integrity violation"() {
        given:
        Payment patchPayment = createTestPayment(1L)
        and:
        paymentService.updatePayment(1L, patchPayment) >> { throw new org.springframework.dao.DataIntegrityViolationException("Duplicate payment") }

        when:
        controller.updatePayment(1L, patchPayment)

        then:
        def ex = thrown(ResponseStatusException)
        ex.statusCode == HttpStatus.CONFLICT
        ex.reason.contains("Duplicate payment")
    }

    def "updatePayment throws 400 on validation exception"() {
        given:
        Payment patchPayment = createTestPayment(1L)
        and:
        paymentService.updatePayment(1L, patchPayment) >> { throw new jakarta.validation.ValidationException("Invalid data") }
    ...
    def "updatePayment throws 400 on illegal argument exception"() {
        given:
        Payment patchPayment = createTestPayment(1L)
        and:
        paymentService.updatePayment(1L, patchPayment) >> { throw new IllegalArgumentException("Invalid input") }
    ...
    def "updatePayment throws 500 on unexpected exception"() {
        given:
        Payment patchPayment = createTestPayment(1L)
        and:
        paymentService.updatePayment(1L, patchPayment) >> { throw new RuntimeException("Unexpected error") }
    ...
    def "updatePayment throws 404 when payment not found"() {
        given:
        Payment patchPayment = createTestPayment(999L)
        and:
        paymentService.updatePayment(999L, patchPayment) >> { throw new org.springframework.web.server.ResponseStatusException(HttpStatus.NOT_FOUND, "Payment not found: 999") }

        when:
        controller.updatePayment(999L, patchPayment)

        then:
        def ex = thrown(ResponseStatusException)
        ex.statusCode == HttpStatus.NOT_FOUND
        ex.reason.contains("Payment not found")
    }

    def "insertPayment returns created payment with 201 status"() {
        given:
        Payment newPayment = createTestPayment(0L)
        Payment savedPayment = createTestPayment(10L)
        and:
        paymentService.insertPayment(newPayment) >> savedPayment

        when:
        ResponseEntity<Payment> response = controller.insertPayment(newPayment)
    ...
    def "insertPayment throws 409 on data integrity violation"() {
        given:
        Payment newPayment = createTestPayment(0L)
        and:
        paymentService.insertPayment(newPayment) >> { throw new org.springframework.dao.DataIntegrityViolationException("Duplicate payment") }

        when:
        controller.insertPayment(newPayment)
    ...
    def "insertPayment throws 400 on validation exception"() {
        given:
        Payment newPayment = createTestPayment(0L)
        and:
        paymentService.insertPayment(newPayment) >> { throw new jakarta.validation.ValidationException("Invalid data") }
    ...
    def "insertPayment throws 400 on illegal argument exception"() {
        given:
        Payment newPayment = createTestPayment(0L)
        and:
        paymentService.insertPayment(newPayment) >> { throw new IllegalArgumentException("Invalid input") }
    ...
    def "insertPayment throws 500 on unexpected exception"() {
        given:
        Payment newPayment = createTestPayment(0L)
        and:
        paymentService.insertPayment(newPayment) >> { throw new RuntimeException("Unexpected error") }
    ...
    def "deleteByPaymentId returns deleted payment when found"() {
        given:
        Payment existingPayment = createTestPayment(1L)
        and:
        paymentService.findByPaymentId(1L) >> Optional.of(existingPayment)
        paymentService.deleteByPaymentId(1L) >> true

        when:
        ResponseEntity<Payment> response = controller.deleteByPaymentId(1L)
    ...
    def "deleteByPaymentId throws 404 when payment not found"() {
        given:
        paymentService.findByPaymentId(999L) >> Optional.empty()

        when:
        controller.deleteByPaymentId(999L)
    ...
    def "deleteByPaymentId throws 500 on unexpected exception during deletion"() {
        given:
        Payment existingPayment = createTestPayment(1L)
        and:
        paymentService.findByPaymentId(1L) >> Optional.of(existingPayment)
        paymentService.deleteByPaymentId(1L) >> { throw new RuntimeException("Delete failed") }

        when:
        controller.deleteByPaymentId(1L)

        then:
        def ex = thrown(ResponseStatusException)
        ex.statusCode == HttpStatus.INTERNAL_SERVER_ERROR
        ex.reason.contains("Failed to delete payment")
    }

    def "deleteByPaymentId throws 500 on unexpected exception during find"() {
        given:
        paymentService.findByPaymentId(1L) >> { throw new RuntimeException("Find failed") }
    ...
    def "selectAllPayments handles payments sorted by transaction date descending"() {
        given:
        Payment payment1 = createTestPayment(1L, "checking_brian", "visa_brian", new BigDecimal("100.00"))
        payment1.transactionDate = new Date(System.currentTimeMillis() - 86400000) // Yesterday
        Payment payment2 = createTestPayment(2L, "savings_brian", "amex_brian", new BigDecimal("200.00"))
        payment2.transactionDate = new Date(System.currentTimeMillis()) // Today
        List<Payment> payments = [payment2, payment1] // Service returns sorted list
        and:
        paymentService.findAllPayments() >> payments
        ...
        response.body[0].paymentId == 2L // Most recent first
        response.body[1].paymentId == 1L
    }
