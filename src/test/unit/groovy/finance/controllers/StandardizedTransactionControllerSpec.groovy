...
class StandardizedTransactionControllerSpec extends Specification {

    StandardizedTransactionService standardizedTransactionService = Mock()
    ...
    def "findTransaction returns transaction when found"() {
        ...
        response.statusCode == HttpStatus.OK
        response.body.guid == guid
        response.body.accountNameOwner == "legacy_account"
    }

    def "findTransaction throws 404 when not found"() {
        given:
        ...
        ex.reason.contains("Transaction not found, guid: $guid")
        1 * meterService.incrementTransactionRestSelectNoneFoundCounter("unknown")
    }

    def "updateTransaction updates existing transaction"() {
        given:
        String guid = "legacy-update-guid"
        Transaction existing = createValidTransaction(guid, "old_account")
        ...
        when:
        ResponseEntity<Transaction> response = controller.updateTransaction(guid, update)

        then:
        response.statusCode == HttpStatus.OK
        response.body.guid == guid
        response.body.accountNameOwner == "new_account"
    }

    def "updateTransaction throws 404 when not found"() {
        given:
    ...
    def "updateTransaction throws 500 on service error"() {
        given:
        String guid = "legacy-update-error"
        ...
        def ex = thrown(org.springframework.web.server.ResponseStatusException)
        ex.statusCode == HttpStatus.INTERNAL_SERVER_ERROR
        ex.reason.contains("Failed to update transaction")
    }
    ...
    def "insertTransaction creates transaction with 201"() {
        given:
        Transaction input = createValidTransaction("insert-guid", "insert_account")
        ...
        then:
        response.statusCode == HttpStatus.CREATED
        response.body.transactionId == 999L
        response.body.guid == "insert-guid"
    }

    def "insertTransaction throws 409 on data integrity violation"() {
        given:
        ...
        ex.reason.contains("Duplicate transaction found")
    }

    def "insertTransaction throws 400 on validation error"() {
        given:
    ...
    def "insertTransaction throws 500 on unexpected error"() {
        ...
        def ex = thrown(org.springframework.web.server.ResponseStatusException)
        ex.statusCode == HttpStatus.INTERNAL_SERVER_ERROR
        ex.reason.contains("Unexpected error: Unexpected database error")
    }
    ...
    def "insertFutureTransaction creates future transaction with 201"() {
        ...
        futureTransaction.transactionDate = Date.valueOf("2024-12-31")
    ...
    def "deleteTransaction deletes existing transaction"() {
        given:
        ...
        response.statusCode == HttpStatus.OK
        response.body.guid == guid
        response.body.accountNameOwner == "delete_legacy_account"
    }

    def "deleteTransaction throws 404 when not found"() {
        given:
    ...
    def "deleteTransaction throws 404 when deletion fails"() {
        given:
    ...
    private Transaction createValidTransaction(String guid, String accountNameOwner) {
        Transaction transaction = new Transaction(
            ...
            Date.valueOf("2024-01-01"),          // transactionDate
