...
-- Account --
-------------
CREATE TABLE IF NOT EXISTS perf.t_account
(
    ...
    billing_statement_close_day SMALLINT      NULL,
    billing_grace_period_days   SMALLINT      NULL,
...
CREATE INDEX IF NOT EXISTS idx_account_type ON perf.t_account(account_type);
CREATE INDEX IF NOT EXISTS idx_account_active_type ON perf.t_account(active_status, account_type);
CREATE INDEX IF NOT EXISTS idx_account_owner ON perf.t_account(owner);

----------------------------
-- Validation Amount Date --
----------------------------
CREATE TABLE IF NOT EXISTS perf.t_validation_amount
(
    ...
    CONSTRAINT fk_validation_account_id FOREIGN KEY (owner, account_id) REFERENCES perf.t_account (owner, account_id) ON UPDATE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_validation_amount_owner ON perf.t_validation_amount(owner);

----------
-- User --
----------
CREATE TABLE IF NOT EXISTS perf.t_user
(
...
-- Role --
----------
CREATE TABLE IF NOT EXISTS perf.t_role
(
...
-- Category --
--------------
CREATE TABLE IF NOT EXISTS perf.t_category
(
...
CREATE INDEX IF NOT EXISTS idx_category_owner ON perf.t_category(owner);

-----------------
-- Description --
-----------------
CREATE TABLE IF NOT EXISTS perf.t_description
(
...
CREATE INDEX IF NOT EXISTS idx_description_owner ON perf.t_description(owner);

---------------------------
-- TransactionCategories --
---------------------------
CREATE TABLE IF NOT EXISTS perf.t_transaction_categories
(
...
CREATE INDEX IF NOT EXISTS idx_transaction_categories_owner ON perf.t_transaction_categories(owner);

-------------------
-- ReceiptImage  --
-------------------
CREATE TABLE IF NOT EXISTS perf.t_receipt_image
(
...
CREATE INDEX IF NOT EXISTS idx_receipt_image_owner ON perf.t_receipt_image(owner);

-----------------
-- Transaction --
-----------------
CREATE TABLE IF NOT EXISTS perf.t_transaction
(
    ...
    CONSTRAINT ck_reoccurring_type CHECK (reoccurring_type IN
                                          ('annually', 'biannually', 'fortnightly', 'monthly', 'quarterly', 'onetime',
                                           'undefined')),
    CONSTRAINT fk_account_id_account_name_owner FOREIGN KEY (owner, account_id, account_name_owner, account_type) REFERENCES perf.t_account (owner, account_id, account_name_owner, account_type) ON UPDATE CASCADE,
    CONSTRAINT fk_receipt_image FOREIGN KEY (receipt_image_id) REFERENCES perf.t_receipt_image (receipt_image_id) ON UPDATE CASCADE,
    CONSTRAINT fk_category_name FOREIGN KEY (owner, category) REFERENCES perf.t_category (owner, category_name) ON UPDATE CASCADE,
    CONSTRAINT fk_description_name FOREIGN KEY (owner, description) REFERENCES perf.t_description (owner, description_name) ON UPDATE CASCADE
);

-- Circular FK: t_receipt_image -> t_transaction (must be added after t_transaction exists)
ALTER TABLE perf.t_receipt_image
    DROP CONSTRAINT IF EXISTS fk_transaction;
ALTER TABLE perf.t_receipt_image
    ADD CONSTRAINT fk_transaction FOREIGN KEY (owner, transaction_id) REFERENCES perf.t_transaction (owner, transaction_id) ON UPDATE CASCADE;

CREATE INDEX IF NOT EXISTS idx_transaction_owner ON perf.t_transaction(owner);
CREATE INDEX IF NOT EXISTS idx_transaction_account_lookup ON perf.t_transaction(account_name_owner, active_status, transaction_date DESC);
CREATE INDEX IF NOT EXISTS idx_transaction_account_name_owner ON perf.t_transaction(account_name_owner);
CREATE INDEX IF NOT EXISTS idx_transaction_date ON perf.t_transaction(transaction_date);
CREATE INDEX IF NOT EXISTS idx_transaction_transaction_state ON perf.t_transaction(transaction_state);

--------------------------
-- Pending Transaction  --
--------------------------
CREATE TABLE IF NOT EXISTS perf.t_pending_transaction
(
    ...
    CONSTRAINT fk_pending_account FOREIGN KEY (owner, account_name_owner) REFERENCES perf.t_account (owner, account_name_owner) ON UPDATE CASCADE,
    CONSTRAINT ck_review_status CHECK (review_status IN ('pending', 'approved', 'rejected'))
);

CREATE INDEX IF NOT EXISTS idx_pending_transaction_owner ON perf.t_pending_transaction(owner);

-------------
-- Payment --
-------------
CREATE TABLE IF NOT EXISTS perf.t_payment
(
    ...
    CONSTRAINT fk_payment_guid_source FOREIGN KEY (guid_source) REFERENCES perf.t_transaction (guid) ON UPDATE CASCADE,
    CONSTRAINT fk_payment_guid_destination FOREIGN KEY (guid_destination) REFERENCES perf.t_transaction (guid) ON UPDATE CASCADE,
    CONSTRAINT fk_payment_source_account FOREIGN KEY (owner, source_account) REFERENCES perf.t_account (owner, account_name_owner) ON UPDATE CASCADE,
    CONSTRAINT fk_payment_destination_account FOREIGN KEY (owner, destination_account) REFERENCES perf.t_account (owner, account_name_owner) ON UPDATE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_payment_owner ON perf.t_payment(owner);

--------------
-- Transfer --
--------------
CREATE TABLE IF NOT EXISTS perf.t_transfer
(
    ...
    CONSTRAINT fk_transfer_guid_source FOREIGN KEY (guid_source) REFERENCES perf.t_transaction (guid) ON UPDATE CASCADE,
    CONSTRAINT fk_transfer_guid_destination FOREIGN KEY (guid_destination) REFERENCES perf.t_transaction (guid) ON UPDATE CASCADE,
    CONSTRAINT fk_source_account FOREIGN KEY (owner, source_account) REFERENCES perf.t_account (owner, account_name_owner) ON UPDATE CASCADE,
    CONSTRAINT fk_destination_account FOREIGN KEY (owner, destination_account) REFERENCES perf.t_account (owner, account_name_owner) ON UPDATE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_transfer_owner ON perf.t_transfer(owner);

---------------
-- Parameter --
---------------
CREATE TABLE IF NOT EXISTS perf.t_parameter
(
...
CREATE INDEX IF NOT EXISTS idx_parameter_owner ON perf.t_parameter(owner);

----------------------
-- Medical Provider --
----------------------
CREATE TABLE IF NOT EXISTS perf.t_medical_provider
(
...
CREATE INDEX IF NOT EXISTS idx_medical_provider_name ON perf.t_medical_provider(provider_name);
CREATE INDEX IF NOT EXISTS idx_medical_provider_type ON perf.t_medical_provider(provider_type);
CREATE INDEX IF NOT EXISTS idx_medical_provider_network ON perf.t_medical_provider(network_status, provider_type);

INSERT INTO perf.t_medical_provider (provider_name, provider_type, specialty, network_status)
...
WHERE NOT EXISTS (
    SELECT 1 FROM perf.t_medical_provider p WHERE p.provider_name = v.provider_name
);

-------------------
-- Family Member --
-------------------
CREATE TABLE IF NOT EXISTS perf.t_family_member
(
...
CREATE INDEX IF NOT EXISTS idx_family_member_owner ON perf.t_family_member(owner);
CREATE INDEX IF NOT EXISTS idx_family_member_relationship ON perf.t_family_member(owner, relationship);

---------------------
-- Medical Expense --
---------------------
CREATE TABLE IF NOT EXISTS perf.t_medical_expense
(
    ...
    CONSTRAINT fk_medical_expense_transaction FOREIGN KEY (owner, transaction_id)
        REFERENCES perf.t_transaction(owner, transaction_id) ON DELETE CASCADE,
    CONSTRAINT fk_medical_expense_provider FOREIGN KEY (provider_id)
        REFERENCES perf.t_medical_provider(provider_id),
    CONSTRAINT fk_medical_expense_family_member FOREIGN KEY (owner, family_member_id)
        REFERENCES perf.t_family_member (owner, family_member_id) ON UPDATE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_medical_expense_provider ON perf.t_medical_expense(provider_id);
CREATE INDEX IF NOT EXISTS idx_medical_expense_family_member ON perf.t_medical_expense(family_member_id);
CREATE INDEX IF NOT EXISTS idx_medical_expense_service_date ON perf.t_medical_expense(service_date);
CREATE INDEX IF NOT EXISTS idx_medical_expense_claim_status ON perf.t_medical_expense(claim_status);
CREATE INDEX IF NOT EXISTS idx_medical_expense_active ON perf.t_medical_expense(active_status, service_date);
CREATE INDEX IF NOT EXISTS idx_medical_expense_owner ON perf.t_medical_expense(owner);
