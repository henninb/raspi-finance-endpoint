-- Remove obsolete account_name_owner column from t_payment (H2 version)
...
UPDATE func.t_payment SET destination_account = LOWER(destination_account) WHERE destination_account IS NOT NULL;
UPDATE func.t_payment SET source_account = LOWER(source_account) WHERE source_account IS NOT NULL;
...
WHERE NOT EXISTS (
    SELECT 1 FROM func.t_account a WHERE a.account_name_owner = p.destination_account
)
AND EXISTS (
    SELECT 1 FROM func.t_account a WHERE a.account_name_owner = REPLACE(p.destination_account, '-', '_')
);

UPDATE func.t_payment p
SET source_account = REPLACE(p.source_account, '-', '_')
WHERE NOT EXISTS (
    SELECT 1 FROM func.t_account a WHERE a.account_name_owner = p.source_account
)
AND EXISTS (
    SELECT 1 FROM func.t_account a WHERE a.account_name_owner = REPLACE(p.source_account, '-', '_')
);
...
-- H2 version: check if constraint exists using INFORMATION_SCHEMA
-- Note: H2 will throw error if constraint already exists, so we need to be careful
ALTER TABLE func.t_payment
