-- V01: Consolidated schema for functional tests (H2 MODE=PostgreSQL)
-- Based on prod V19 consolidated schema, adapted for H2 syntax.
-- Incorporates: V02 (token_blacklist), V03 (transfer version), V04 (date_closed nullable),
--               V05 (payment unique constraint), V06 (t_reward), V07 (tax_bucket)
...
CREATE INDEX IF NOT EXISTS idx_account_type ON t_account(account_type);
CREATE INDEX IF NOT EXISTS idx_account_active_type ON t_account(active_status, account_type);
CREATE INDEX IF NOT EXISTS idx_account_owner ON t_account(owner);

----------------------------
-- Validation Amount Date --
----------------------------
CREATE TABLE IF NOT EXISTS t_validation_amount
(
    ...
    CONSTRAINT fk_validation_account_id FOREIGN KEY (owner, account_id) REFERENCES t_account (owner, account_id) ON UPDATE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_validation_amount_owner ON t_validation_amount(owner);
...
CREATE INDEX IF NOT EXISTS idx_category_owner ON t_category(owner);
...
CREATE INDEX IF NOT EXISTS idx_description_owner ON t_description(owner);
...
CREATE INDEX IF NOT EXISTS idx_transaction_categories_owner ON t_transaction_categories(owner);
...
(
    receipt_image_id  BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    transaction_id    BIGINT                             NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_receipt_image_owner ON t_receipt_image(owner);
...
(
    ...
    CONSTRAINT ck_reoccurring_type CHECK (reoccurring_type IN
                                          ('annually', 'biannually', 'fortnightly', 'monthly', 'quarterly', 'onetime',
                                           'undefined')),
    CONSTRAINT fk_account_id_account_name_owner FOREIGN KEY (owner, account_id, account_name_owner, account_type) REFERENCES t_account (owner, account_id, account_name_owner, account_type) ON UPDATE CASCADE,
    CONSTRAINT fk_receipt_image FOREIGN KEY (receipt_image_id) REFERENCES t_receipt_image (receipt_image_id) ON UPDATE CASCADE,
    CONSTRAINT fk_category_name FOREIGN KEY (owner, category) REFERENCES t_category (owner, category_name) ON UPDATE CASCADE,
    CONSTRAINT fk_description_name FOREIGN KEY (owner, description) REFERENCES t_description (owner, description_name) ON UPDATE CASCADE
);
...
ALTER TABLE t_receipt_image
    ADD CONSTRAINT fk_transaction FOREIGN KEY (owner, transaction_id) REFERENCES t_transaction (owner, transaction_id) ON UPDATE CASCADE;

CREATE INDEX IF NOT EXISTS idx_transaction_owner ON t_transaction(owner);
CREATE INDEX IF NOT EXISTS idx_transaction_account_lookup ON t_transaction(account_name_owner, active_status, transaction_date DESC);
CREATE INDEX IF NOT EXISTS idx_transaction_account_name_owner ON t_transaction(account_name_owner);
CREATE INDEX IF NOT EXISTS idx_transaction_date ON t_transaction(transaction_date);
CREATE INDEX IF NOT EXISTS idx_transaction_transaction_state ON t_transaction(transaction_state);
...
(
    ...
    date_updated         TIMESTAMP     DEFAULT PARSEDATETIME('1970-01-01 00:00:00.0', 'yyyy-MM-dd HH:mm:ss.S') NOT NULL,
    date_added           TIMESTAMP     DEFAULT PARSEDATETIME('1970-01-01 00:00:00.0', 'yyyy-MM-dd HH:mm:ss.S') NOT NULL,
    CONSTRAINT unique_owner_payment UNIQUE (owner, source_account, destination_account, transaction_date, amount),
    CONSTRAINT fk_payment_guid_source FOREIGN KEY (guid_source) REFERENCES t_transaction (guid) ON UPDATE CASCADE,
    CONSTRAINT fk_payment_guid_destination FOREIGN KEY (guid_destination) REFERENCES t_transaction (guid) ON UPDATE CASCADE,
    CONSTRAINT fk_payment_source_account FOREIGN KEY (owner, source_account) REFERENCES t_account (owner, account_name_owner) ON UPDATE CASCADE,
    CONSTRAINT fk_payment_destination_account FOREIGN KEY (owner, destination_account) REFERENCES t_account (owner, account_name_owner) ON UPDATE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_payment_owner ON t_payment(owner);
...
(
    ...
    CONSTRAINT fk_transfer_guid_source FOREIGN KEY (guid_source) REFERENCES t_transaction (guid) ON UPDATE CASCADE,
    CONSTRAINT fk_transfer_guid_destination FOREIGN KEY (guid_destination) REFERENCES t_transaction (guid) ON UPDATE CASCADE,
    CONSTRAINT fk_source_account FOREIGN KEY (owner, source_account) REFERENCES t_account (owner, account_name_owner) ON UPDATE CASCADE,
    CONSTRAINT fk_destination_account FOREIGN KEY (owner, destination_account) REFERENCES t_account (owner, account_name_owner) ON UPDATE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_transfer_owner ON t_transfer(owner);
...
(
    parameter_id       BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_parameter_owner ON t_parameter(owner);

----------------------
-- Medical Provider --
----------------------
CREATE TABLE IF NOT EXISTS t_medical_provider
(
...
CREATE INDEX IF NOT EXISTS idx_medical_provider_name ON t_medical_provider(provider_name);
CREATE INDEX IF NOT EXISTS idx_medical_provider_type ON t_medical_provider(provider_type);
CREATE INDEX IF NOT EXISTS idx_medical_provider_network ON t_medical_provider(network_status, provider_type);

INSERT INTO t_medical_provider (provider_name, provider_type, specialty, network_status)
...
WHERE NOT EXISTS (
    SELECT 1 FROM t_medical_provider p WHERE p.provider_name = v.provider_name
);
...
CREATE INDEX IF NOT EXISTS idx_family_member_owner ON t_family_member(owner);
CREATE INDEX IF NOT EXISTS idx_family_member_relationship ON t_family_member(owner, relationship);

---------------------
-- Medical Expense --
---------------------
CREATE TABLE IF NOT EXISTS t_medical_expense
(
    medical_expense_id          BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    ...
    CONSTRAINT fk_medical_expense_transaction FOREIGN KEY (owner, transaction_id)
        REFERENCES t_transaction(owner, transaction_id) ON DELETE CASCADE,
    ...
    CONSTRAINT fk_medical_expense_family_member FOREIGN KEY (owner, family_member_id)
        REFERENCES t_family_member (owner, family_member_id) ON UPDATE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_medical_expense_provider ON t_medical_expense(provider_id);
CREATE INDEX IF NOT EXISTS idx_medical_expense_family_member ON t_medical_expense(family_member_id);
CREATE INDEX IF NOT EXISTS idx_medical_expense_service_date ON t_medical_expense(service_date);
CREATE INDEX IF NOT EXISTS idx_medical_expense_claim_status ON t_medical_expense(claim_status);
CREATE INDEX IF NOT EXISTS idx_medical_expense_active ON t_medical_expense(active_status, service_date);
CREATE INDEX IF NOT EXISTS idx_medical_expense_owner ON t_medical_expense(owner);

---------------------
-- Token Blacklist --
---------------------
CREATE TABLE IF NOT EXISTS t_token_blacklist (
    token_blacklist_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_token_blacklist_expires_at
    ON t_token_blacklist (expires_at);
...
(
    ...
    CONSTRAINT fk_reward_account_id FOREIGN KEY (account_id) REFERENCES t_account (account_id) ON DELETE CASCADE
);
