-- V01: Consolidated schema for integration tests (H2 MODE=PostgreSQL)
-- Based on prod V19 consolidated schema, adapted for H2 syntax.
-- Incorporates: V02 (transfer version), V03 (date_closed nullable),
--               V04 (payment unique constraint), V05 (t_reward), V06 (tax_bucket)
...
(
    ...
    CONSTRAINT fk_validation_account_id FOREIGN KEY (owner, account_id)
        REFERENCES t_account (owner, account_id) ON UPDATE CASCADE
);
...
ALTER TABLE t_receipt_image
    ...
    REFERENCES t_transaction (owner, transaction_id) ON UPDATE CASCADE;
