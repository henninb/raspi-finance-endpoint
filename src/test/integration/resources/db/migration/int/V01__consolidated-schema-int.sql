-- V01: Consolidated schema for integration tests (H2)
...
CREATE INDEX IF NOT EXISTS idx_account_owner ON int.t_account(owner);
...
CREATE INDEX IF NOT EXISTS idx_category_owner ON int.t_category(owner);
...
CREATE INDEX IF NOT EXISTS idx_description_owner ON int.t_description(owner);
...
CREATE INDEX IF NOT EXISTS idx_transaction_categories_owner ON int.t_transaction_categories(owner);
...
CREATE INDEX IF NOT EXISTS idx_receipt_image_owner ON int.t_receipt_image(owner);
...
(
    ...
    CONSTRAINT ck_reoccurring_type CHECK (reoccurring_type IN
                                          ('annually', 'biannually', 'fortnightly', 'monthly', 'quarterly', 'onetime',
                                           'undefined')),
    CONSTRAINT fk_account_id_account_name_owner FOREIGN KEY (owner, account_id, account_name_owner, account_type) REFERENCES int.t_account (owner, account_id, account_name_owner, account_type) ON UPDATE CASCADE,
    CONSTRAINT fk_receipt_image FOREIGN KEY (receipt_image_id) REFERENCES int.t_receipt_image (receipt_image_id) ON UPDATE CASCADE,
    CONSTRAINT fk_category_name FOREIGN KEY (owner, category) REFERENCES int.t_category (owner, category_name) ON UPDATE CASCADE,
    CONSTRAINT fk_description_name FOREIGN KEY (owner, description) REFERENCES int.t_description (owner, description_name) ON UPDATE CASCADE
);
...
(
    ...
    CONSTRAINT fk_pending_account FOREIGN KEY (owner, account_name_owner) REFERENCES int.t_account (owner, account_name_owner) ON UPDATE CASCADE,
    CONSTRAINT ck_review_status CHECK (review_status IN ('pending', 'approved', 'rejected'))
);

CREATE INDEX IF NOT EXISTS idx_pending_transaction_owner ON int.t_pending_transaction(owner);
...
CREATE INDEX IF NOT EXISTS idx_parameter_owner ON int.t_parameter(owner);
...
INSERT INTO int.t_medical_provider (provider_name, provider_type, specialty, network_status)
...
WHERE NOT EXISTS (
    SELECT 1 FROM int.t_medical_provider p WHERE p.provider_name = v.provider_name
);
...
(
    ...
    CONSTRAINT fk_medical_expense_transaction FOREIGN KEY (owner, transaction_id)
        REFERENCES int.t_transaction(owner, transaction_id) ON DELETE CASCADE,
    CONSTRAINT fk_medical_expense_provider FOREIGN KEY (provider_id)
        REFERENCES int.t_medical_provider(provider_id),
    CONSTRAINT fk_medical_expense_family_member FOREIGN KEY (owner, family_member_id)
        REFERENCES int.t_family_member (owner, family_member_id) ON UPDATE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_medical_expense_provider ON int.t_medical_expense(provider_id);
CREATE INDEX IF NOT EXISTS idx_medical_expense_family_member ON int.t_medical_expense(family_member_id);
CREATE INDEX IF NOT EXISTS idx_medical_expense_service_date ON int.t_medical_expense(service_date);
